...
void updateMemoryAndDisplay(int offset, int value) {
	...
	cout << "Memory Updated -> Address: " << address << " | Value: " << value << endl;
}
...
void ProcessBFormat(BFormat& value, int& counter) {
	...
	int rs1_value = registers[extractRegisterNumber(value.rs1.second)];
	int rs2_value = registers[extractRegisterNumber(value.rs2.second)];
...
void ProcessSFormat(SFormat& value, Memory& memory) {
	...
	int address = (value.imm);  // Adjusted for 4-byte addressing
	//memory[address + 1] = (x2 >> 8) & 0xFF; // Store upper byte
	if (value.opcode == "SB") {
	//	memory.store(address, value.rs2.first & 0xFF);
		value.rs2.first = value.rs1.first;
		value.rs2.second = value.rs2.second;
		updateMemoryAndDisplay(address, value.rs2.first & 0xFF); // Byte
	}
	else if (value.opcode == "SH") {
	//	memory.store(address, value.rs2.first & 0xFFFF);
		value.rs2.first = value.rs1.first;
		value.rs2.second = value.rs2.second;
		updateMemoryAndDisplay(address, value.rs2.first & 0xFFFF); // Halfword
	}
	else if (value.opcode == "SW") {
	//	memory.store(address, value.rs2.first);
		value.rs2.first = value.rs1.first;
		value.rs2.second = value.rs2.second;
		updateMemoryAndDisplay(address, value.rs2.first); // Word
	}
	else {
...
int main()
{
	...
	for (int i = 0; i < num; i++) {
		...
		cin >> RegisterValue;
		cout << "Please enter your desired starting address: " << endl;
		cin >> startAddress;
		memory.store(startAddress, RegisterValue);
     	cout << "Memory Initialized from Address " << startAddress << endl;
		registers[extractRegisterNumber(RegisterN)] = RegisterValue;
	}
	readAndProcessAssemblyFile("Bonus_Case3.txt");
